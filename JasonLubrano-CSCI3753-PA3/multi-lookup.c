...
int SHARED_ARRAY_ADD_ITEM(SHARED_ARRAY* s_array, void* new_contains){
    ...
    if(debug){
    	printf("Shared array add item was successful, returning 0\n");
    }
...
/* handles all the request threads */
/* passing the address of the thread info */
void* REQUEST_THREAD(void* threadarg){
    ...
    */
    ...
    if(!output2){
		perror("Error Opening Output service File"); /* no need for debug message */
    }
    ...
    for(i = 0; i < REQ_TH_DATA_STRUCT->inFiles.size; i++){
        ...
        if (debug){
        	printf("now the file is being read\n");
        }
            ...
            while(!completed){
                ...
                if (!SHARED_ARRAY_TEST_FULL(s_array)){
                    ...
                    */
                    ...
                    if(debug){
                    	printf("Adding contains to SHARED_ARRAY:%s\n", contains);
                    }
    ...
    if(debug){
    	printf("%s is closing, requester thread #%ld says goodnight\n", __func__, threadNum);
	}
...
/* thread that handles the resolve threads */
...
int main(int argc, char* argv[]){
    ...
    */
    ...
    for(thread_counter2=0; thread_counter2<num_res_ths; thread_counter2++){
    	if (debug){
    		printf("welcome RESOLVER Thread number %ld to the world\n", thread_counter2);
    	}
    ...
    if (debug){
    	printf("The requester threads are done and joined\n");
    }
